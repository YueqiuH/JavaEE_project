-- ============================================
-- 成员 D：基础数据种子（对已建库执行一次；与 baseline/init.sql 末尾种子一致）
-- 覆盖：department / major / grade / user(教职工档案) / student / enrollment / news / forum_*
-- 依赖：V20260717100000__base_profile_columns.sql
-- ============================================
...
-- ---------- 教职工档案（统一密码: 123321） ----------
UPDATE `user` SET `real_name` = '刘一鸣', `gender` = 1, `title` = '讲师',   `position` = NULL,     `dept_id` = @d_cs, `phone` = '13800001001', `email` = 'liuym@campus.edu'  WHERE `username` = '700001';
UPDATE `user` SET `real_name` = '宋建国', `gender` = 1, `title` = NULL,     `position` = '教务干事', `dept_id` = @d_cs, `phone` = '13800001002', `email` = 'songjg@campus.edu' WHERE `username` = '800001';
...
-- ---------- 学生档案（60 名，覆盖 3 个年级 / 10 个专业 / 12 个省份） ----------
