...
export const canAccessService = (service, userPermissions = []) => {
  const granted = new Set(userPermissions)
  if (service.permission && granted.has(service.permission)) return true
  if (service.broadPermission && granted.has(service.broadPermission)) return true
  if (service.permissions?.length) return service.permissions.some(p => granted.has(p))
  if (!service.permission && !service.permissions?.length && !service.roles) return true
  return false
}
