<template>
  <section class="office-page">
    <header class="office-page__header">
      <div><h1>固定资产管理与申领</h1><p>教职工从可用台账了解并申请现有资产，也可提交其他购置申请。</p></div>
      <div class="office-page__actions">
        ...
        <el-button v-if="canApply" type="primary" @click="openDialog('apply')">提交购置申请</el-button>
    ...
    <template v-else>
      ...
      <el-alert v-if="activeTab==='inventory'" class="inventory-tip" type="info" :closable="false" show-icon title="这里只显示仍在库且数量大于 0 的现有资产；点击“申请领用”即可提交申请，全部领用后会自动退出台账。" />
      <el-card class="office-page__section" shadow="never">
        <el-table :data="rows" v-loading="loading" stripe :empty-text="emptyText">
          ...
          <el-table-column v-if="activeTab==='inventory'&&(canApply||canManage)" label="操作" min-width="180">
            <template #default="scope">
              <el-button v-if="canApply" link type="primary" @click="openAvailableApply(scope.row)">申请领用</el-button>
              <el-button v-if="canManage" link type="danger" @click="remove(scope.row)">删除</el-button>
            </template>
          </el-table-column>
          <el-table-column v-else-if="canManage&&activeTab==='applications'" label="管理" min-width="210">
            <template #default="scope">
    ...
    <el-dialog v-model="visible" :title="dialogTitle" width="min(520px,92vw)">
      <el-form label-position="top">
        <el-form-item label="资产名称" required><el-input v-model="form.assetName" :disabled="dialogMode==='available'" /></el-form-item>
        <el-form-item label="资产类型" required><el-select v-model="form.assetType" style="width:100%" :disabled="dialogMode==='available'"><el-option v-for="value in ['设备','办公用品','其他']" :key="value" :value="value" /></el-select></el-form-item>
        <el-form-item label="数量" required><el-input-number v-model="form.quantity" :min="1" :max="dialogMode==='available'?selectedInventory?.quantity:undefined" /></el-form-item>
        <el-form-item label="所属部门 ID"><el-input-number v-model="form.deptId" :min="1" :disabled="dialogMode==='available'" /></el-form-item>
      </el-form>
...
const dialogTitle = computed(() => ({ apply: '提交资产购置申请', available: '申请领用现有资产', save: '新增资产' }[dialogMode.value]))
...
const openDialog = (mode) => { dialogMode.value = mode; selectedInventory.value = null; Object.assign(form, { assetName: '', assetType: '设备', quantity: 1, deptId: deptId.value || 1 }); visible.value = true }
const openAvailableApply = (row) => { dialogMode.value = 'available'; selectedInventory.value = row; Object.assign(form, { assetName: row.assetName, assetType: row.assetType, quantity: 1, deptId: row.deptId }); visible.value = true }
const submit = async () => {
  if (!form.assetName.trim() || !form.quantity) return ElMessage.warning('请填写资产名称和数量')
  if (dialogMode.value === 'available') {
    await assetAPI.applyAvailable(selectedInventory.value.assetId, form.quantity)
    activeTab.value = 'mine'
    ElMessage.success('领用申请已提交，可在“我的申请”查看进度')
