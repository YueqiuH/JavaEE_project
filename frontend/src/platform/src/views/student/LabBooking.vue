  ...
  <section class="lab-workspace">
    <header class="workspace-header">
      <div>
        ...
        <h1>{{ isTeacher ? '实验室开放管理' : '实验室当日预约' }}</h1>
        <p>{{ isTeacher ? '维护实验室容量与开放状态，查看学生使用情况' : '预约今日使用名额并完成签到、签退' }}</p>
      </div>
      <el-button v-if="isTeacher && activeTab === 'labs' && canManageLab" type="primary" :icon="Plus" @click="openLabDialog()">新建实验室</el-button>
    </header>
    ...
    <div v-else class="surface-panel data-panel">
      ...
      <div class="panel-toolbar">
        ...
        <div class="filter-actions">
          ...
          <el-select v-else-if="isTeacher && activeTab === 'labs'" v-model="statusFilter" placeholder="全部状态" clearable @change="resetPageAndLoad"><el-option label="开放" value="OPEN" /><el-option label="关闭" value="MAINTENANCE" /></el-select>
      ...
      <template v-else-if="activeTab === 'bookings' || activeTab === 'managed'">
        <el-table v-if="loading || records.length" v-loading="loading" :data="records" row-key="bookingId" class="desktop-table" @row-click="openBookingDetail">
          ...
          <el-table-column v-if="isTeacher" label="预约学生" width="150"><template #default="{ row }">{{ row.studentName }}<small>{{ row.studentNo }}</small></template></el-table-column>
          ...
          <el-table-column label="操作" width="230" fixed="right"><template #default="{ row }"><div class="row-actions" @click.stop><el-button link :icon="View" @click="openBookingDetail(row)">详情</el-button><el-button v-if="isStudent && row.status === 'RESERVED' && canCheckIn" link type="primary" :icon="CircleCheck" @click="checkIn(row)">签到</el-button><el-button v-if="isStudent && row.status === 'RESERVED' && canCancel" link type="danger" :icon="Close" @click="cancel(row)">取消</el-button><el-button v-if="isStudent && row.status === 'CHECKED_IN' && canCheckOut" link type="primary" :icon="SwitchButton" @click="checkOut(row)">签退</el-button><el-button v-if="isTeacher && row.status === 'CHECKED_IN'" link type="danger" :icon="SwitchButton" @click="forceCheckOut(row)">强制签退</el-button></div></template></el-table-column>
        </el-table>
        <div class="mobile-records"><article v-for="row in records" :key="row.bookingId" class="mobile-record"><span class="record-top"><strong>{{ row.labName }}</strong><el-tag :type="bookingStatusMeta(row.status).type" size="small">{{ bookingStatusMeta(row.status).label }}</el-tag></span><span>{{ formatDate(row.bookingDate) }}<template v-if="isTeacher"> · {{ row.studentName }}</template></span><small>{{ row.bookingNo }}</small><div class="mobile-actions"><el-button size="small" @click="openBookingDetail(row)">详情</el-button><el-button v-if="isStudent && row.status === 'RESERVED'" size="small" type="primary" @click="checkIn(row)">签到</el-button><el-button v-if="isStudent && row.status === 'CHECKED_IN'" size="small" type="primary" @click="checkOut(row)">签退</el-button><el-button v-if="isTeacher && row.status === 'CHECKED_IN'" size="small" type="danger" plain @click="forceCheckOut(row)">强制签退</el-button></div></article><el-empty v-if="!loading && !records.length" class="mobile-empty" :description="emptyText" /></div>
      </template>
...
const panelTitle = computed(() => ({ labs: isTeacher.value ? '我负责的实验室' : '今日开放实验室', bookings: '我的预约记录', notices: '预约通知', managed: '学生预约记录' }[activeTab.value]))
const emptyText = computed(() => ({ labs: isTeacher.value ? '暂未维护实验室' : '暂无开放实验室', bookings: '今日暂无实验室预约', notices: '暂无预约通知', managed: '暂无学生预约记录' }[activeTab.value]))
...
watch([isStudent, isTeacher], ([student, teacher]) => { if (student || teacher) { activeTab.value = 'labs'; resetAndLoad() } }, { immediate: true })
</script>
