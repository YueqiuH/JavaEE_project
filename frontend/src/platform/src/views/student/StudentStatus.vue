  ...
  <section class="status-workspace">
    ...
    <template v-else-if="isTeacher">
      <div class="surface-panel data-panel teacher-panel">
        <el-tabs v-model="reviewStage" class="workspace-tabs review-tabs" @tab-change="resetAndLoad">
    ...
    return () => h(ElTable, { data: props.applications, class: 'desktop-table', rowKey: 'changeId', onRowClick: (row) => emit('detail', row), loading: props.loading }, {
      default: () => [
        ...
        h(ElTableColumn, { label: '操作', width: props.teacher ? 150 : 225, fixed: 'right' }, { default: ({ row }) => h('div', { class: 'row-actions', onClick: (event) => event.stopPropagation() }, [
          ...
          props.teacher ? h(ElButton, { link: true, type: 'primary', icon: Stamp, onClick: () => emit('review', row) }, () => '审核') : null,
        ]) }),
      ],
    })
  },
...
const loadApplications = async () => { if (!isStudent.value && !isTeacher.value) return; loading.value = true; try { const result = isStudent.value ? await listMyStatusChanges({ page: page.value, size: pageSize, status: studentStatus.value || undefined }) : await listStatusChangeReviews({ stage: reviewStage.value, page: page.value, size: pageSize }); applications.value = result.data.records; total.value = result.data.total } finally { loading.value = false } }
